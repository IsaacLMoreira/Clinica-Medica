package view.mdi;

import java.awt.BorderLayout;
import java.awt.Font;
import java.awt.GridBagConstraints;
import java.awt.GridBagLayout;
import java.awt.Insets;

import java.time.LocalDate;
import java.time.format.DateTimeFormatter;
import java.time.format.DateTimeParseException;
import java.time.format.ResolverStyle;

import javax.swing.BorderFactory;
import javax.swing.JButton;
import javax.swing.JInternalFrame;
import javax.swing.JLabel;
import javax.swing.JOptionPane;
import javax.swing.JPanel;
import javax.swing.JTextField;
import javax.swing.SwingConstants;

import model.Paciente;
import repository.PacienteRepository;

public class CadastroPacienteMDI extends JInternalFrame {

    // Campos preenchidos pelo usuário
    private JTextField txtNome;
    private JTextField txtCpf;
    private JTextField txtDataNascimento;
    private JTextField txtTelefone;
    private JTextField txtEmail;
    private JTextField txtConvenio;

    // Botões da tela
    private JButton btnCadastrar;
    private JButton btnLimpar;
    private JButton btnFechar;

    // Responsável por salvar o paciente no banco
    private final PacienteRepository repository;

    // Formato utilizado para leitura da data
    private final DateTimeFormatter formatoData =
            DateTimeFormatter
                    .ofPattern("dd/MM/uuuu")
                    .withResolverStyle(
                            ResolverStyle.STRICT
                    );


    public CadastroPacienteMDI() {

        /*
         * Configura o JInternalFrame:
         *
         * título
         * redimensionável
         * fechável
         * maximizável
         * minimizável
         */
        super(
                "Cadastro de Paciente",
                true,
                true,
                true,
                true
        );

        repository =
                new PacienteRepository();

        configurarJanela();
        inicializarComponentes();
    }


    // Configura a janela interna
    private void configurarJanela() {

        setSize(
                600,
                500
        );

        /*
         * Fecha apenas esta janela interna.
         * A TelaPrincipalMDI continua aberta.
         */
        setDefaultCloseOperation(
                JInternalFrame.DISPOSE_ON_CLOSE
        );
    }


    // Cria e organiza os componentes
    private void inicializarComponentes() {

        setLayout(
                new BorderLayout()
        );

        criarCabecalho();
        criarFormulario();
        criarBotoes();

        /*
         * Pressionar Enter executa
         * o botão Cadastrar.
         */
        getRootPane()
                .setDefaultButton(
                        btnCadastrar
                );
    }


    // =====================================================
    // CABEÇALHO
    // =====================================================

    private void criarCabecalho() {

        JPanel painelCabecalho =
                new JPanel();

        painelCabecalho.setBorder(
                BorderFactory.createEmptyBorder(
                        20,
                        10,
                        15,
                        10
                )
        );


        JLabel lblTitulo =
                new JLabel(
                        "CADASTRO DE PACIENTE",
                        SwingConstants.CENTER
                );


        lblTitulo.setFont(
                new Font(
                        "Arial",
                        Font.BOLD,
                        22
                )
        );


        painelCabecalho.add(
                lblTitulo
        );


        add(
                painelCabecalho,
                BorderLayout.NORTH
        );
    }


    // =====================================================
    // FORMULÁRIO
    // =====================================================

    private void criarFormulario() {

        JPanel painelFormulario =
                new JPanel(
                        new GridBagLayout()
                );


        painelFormulario.setBorder(
                BorderFactory.createEmptyBorder(
                        10,
                        40,
                        10,
                        40
                )
        );


        GridBagConstraints gbc =
                new GridBagConstraints();


        gbc.insets =
                new Insets(
                        7,
                        7,
                        7,
                        7
                );

        gbc.fill =
                GridBagConstraints.HORIZONTAL;


        // Código
        adicionarComponente(
                painelFormulario,
                new JLabel("Código:"),
                gbc,
                0,
                0
        );


        adicionarComponente(
                painelFormulario,
                new JLabel(
                        "Gerado automaticamente"
                ),
                gbc,
                1,
                0
        );


        // Nome
        txtNome =
                new JTextField(25);


        adicionarComponente(
                painelFormulario,
                new JLabel("Nome:"),
                gbc,
                0,
                1
        );


        adicionarComponente(
                painelFormulario,
                txtNome,
                gbc,
                1,
                1
        );


        // CPF
        txtCpf =
                new JTextField(25);


        adicionarComponente(
                painelFormulario,
                new JLabel("CPF:"),
                gbc,
                0,
                2
        );


        adicionarComponente(
                painelFormulario,
                txtCpf,
                gbc,
                1,
                2
        );


        // Data de nascimento
        txtDataNascimento =
                new JTextField(25);


        txtDataNascimento.setToolTipText(
                "Digite no formato dd/MM/yyyy"
        );


        adicionarComponente(
                painelFormulario,
                new JLabel(
                        "Data de nascimento:"
                ),
                gbc,
                0,
                3
        );


        adicionarComponente(
                painelFormulario,
                txtDataNascimento,
                gbc,
                1,
                3
        );


        // Telefone
        txtTelefone =
                new JTextField(25);


        adicionarComponente(
                painelFormulario,
                new JLabel("Telefone:"),
                gbc,
                0,
                4
        );


        adicionarComponente(
                painelFormulario,
                txtTelefone,
                gbc,
                1,
                4
        );


        // E-mail
        txtEmail =
                new JTextField(25);


        adicionarComponente(
                painelFormulario,
                new JLabel("E-mail:"),
                gbc,
                0,
                5
        );


        adicionarComponente(
                painelFormulario,
                txtEmail,
                gbc,
                1,
                5
        );


        // Convênio
        txtConvenio =
                new JTextField(25);


        adicionarComponente(
                painelFormulario,
                new JLabel("Convênio:"),
                gbc,
                0,
                6
        );


        adicionarComponente(
                painelFormulario,
                txtConvenio,
                gbc,
                1,
                6
        );


        add(
                painelFormulario,
                BorderLayout.CENTER
        );
    }


    /*
     * Método auxiliar usado para posicionar
     * os componentes no GridBagLayout.
     */
    private void adicionarComponente(
            JPanel painel,
            java.awt.Component componente,
            GridBagConstraints gbc,
            int coluna,
            int linha) {

        gbc.gridx = coluna;
        gbc.gridy = linha;

        /*
         * A coluna dos campos recebe
         * mais espaço horizontal.
         */
        gbc.weightx =
                coluna == 1
                        ? 1.0
                        : 0.0;


        painel.add(
                componente,
                gbc
        );
    }


    // =====================================================
    // BOTÕES
    // =====================================================

    private void criarBotoes() {

        JPanel painelBotoes =
                new JPanel();


        painelBotoes.setBorder(
                BorderFactory.createEmptyBorder(
                        10,
                        10,
                        20,
                        10
                )
        );


        btnCadastrar =
                new JButton(
                        "Cadastrar"
                );

        btnLimpar =
                new JButton(
                        "Limpar"
                );

        btnFechar =
                new JButton(
                        "Fechar"
                );


        // Define as ações
        btnCadastrar.addActionListener(
                e -> cadastrarPaciente()
        );

        btnLimpar.addActionListener(
                e -> limparCampos()
        );

        btnFechar.addActionListener(
                e -> dispose()
        );


        painelBotoes.add(
                btnCadastrar
        );

        painelBotoes.add(
                btnLimpar
        );

        painelBotoes.add(
                btnFechar
        );


        add(
                painelBotoes,
                BorderLayout.SOUTH
        );
    }


    // =====================================================
    // CADASTRAR
    // =====================================================

    private void cadastrarPaciente() {

        try {

            // Verifica os campos antes do cadastro
            validarCampos();


            /*
             * Converte a data digitada pelo usuário
             * para LocalDate.
             */
            LocalDate dataNascimento =
                    LocalDate.parse(
                            txtDataNascimento
                                    .getText()
                                    .trim(),
                            formatoData
                    );


            // Cria um novo paciente
            Paciente paciente =
                    new Paciente();


            // Copia os dados da tela para o objeto
            paciente.setNome(
                    txtNome
                            .getText()
                            .trim()
            );


            paciente.setCpf(
                    txtCpf
                            .getText()
                            .trim()
            );


            paciente.setDataNascimento(
                    dataNascimento
            );


            paciente.setTelefone(
                    txtTelefone
                            .getText()
                            .trim()
            );


            paciente.setEmail(
                    txtEmail
                            .getText()
                            .trim()
            );


            paciente.setConvenio(
                    txtConvenio
                            .getText()
                            .trim()
            );


            /*
             * Envia o paciente para o repository,
             * que executará o INSERT.
             */
            repository.salvar(
                    paciente
            );


            /*
             * Após salvar, o objeto recebe
             * o ID gerado pelo PostgreSQL.
             */
            JOptionPane.showMessageDialog(
                    this,
                    "Paciente cadastrado com sucesso!\n"
                            + "Código: "
                            + paciente.getId(),
                    "Cadastro realizado",
                    JOptionPane.INFORMATION_MESSAGE
            );


            // Prepara o formulário para outro cadastro
            limparCampos();


        } catch (DateTimeParseException e) {

            JOptionPane.showMessageDialog(
                    this,
                    "Data de nascimento inválida.\n"
                            + "Utilize o formato dd/MM/yyyy.",
                    "Data inválida",
                    JOptionPane.WARNING_MESSAGE
            );


            txtDataNascimento
                    .requestFocus();


        } catch (IllegalArgumentException e) {

            /*
             * Mostra erros encontrados
             * durante a validação.
             */
            JOptionPane.showMessageDialog(
                    this,
                    e.getMessage(),
                    "Dados incompletos",
                    JOptionPane.WARNING_MESSAGE
            );


        } catch (RuntimeException e) {

            /*
             * Trata erros vindos do repository
             * ou do banco de dados.
             */
            JOptionPane.showMessageDialog(
                    this,
                    "Não foi possível cadastrar o paciente.\n"
                            + e.getMessage(),
                    "Erro",
                    JOptionPane.ERROR_MESSAGE
            );
        }
    }


    // =====================================================
    // VALIDAÇÃO
    // =====================================================

    private void validarCampos() {

        if (txtNome
                .getText()
                .trim()
                .isEmpty()) {

            txtNome.requestFocus();

            throw new IllegalArgumentException(
                    "Informe o nome do paciente."
            );
        }


        if (txtCpf
                .getText()
                .trim()
                .isEmpty()) {

            txtCpf.requestFocus();

            throw new IllegalArgumentException(
                    "Informe o CPF do paciente."
            );
        }


        if (txtDataNascimento
                .getText()
                .trim()
                .isEmpty()) {

            txtDataNascimento
                    .requestFocus();

            throw new IllegalArgumentException(
                    "Informe a data de nascimento."
            );
        }


        if (txtTelefone
                .getText()
                .trim()
                .isEmpty()) {

            txtTelefone
                    .requestFocus();

            throw new IllegalArgumentException(
                    "Informe o telefone."
            );
        }


        if (txtEmail
                .getText()
                .trim()
                .isEmpty()) {

            txtEmail
                    .requestFocus();

            throw new IllegalArgumentException(
                    "Informe o e-mail."
            );
        }


        if (txtConvenio
                .getText()
                .trim()
                .isEmpty()) {

            txtConvenio
                    .requestFocus();

            throw new IllegalArgumentException(
                    "Informe o convênio."
            );
        }
    }


    // =====================================================
    // LIMPAR CAMPOS
    // =====================================================

    private void limparCampos() {

        txtNome.setText("");
        txtCpf.setText("");
        txtDataNascimento.setText("");
        txtTelefone.setText("");
        txtEmail.setText("");
        txtConvenio.setText("");

        // Retorna o cursor para o primeiro campo
        txtNome.requestFocus();
    }
}